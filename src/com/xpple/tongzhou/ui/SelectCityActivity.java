...
import com.xpple.tongzhou.view.MyLetterListView;
...
public class SelectCityActivity extends BaseActivity {
    ...
    private ArrayList<City> allCity_lists; // �?有城市列�?
    private ArrayList<City> ShowCity_lists; // �?要显示的城市列表-随搜索�?�改�?
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        cityLayout=(LinearLayout)findViewById(R.id.actionbar_city);
        ...
        personList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
                ...
                setResult(ProjectPublish1Activity.SELECT_CITY_CODE, intent);
                finish();
            }
        });
    ...
    private void  setLight(int firstVisibleItem) {
    	if(ShowCity_lists.size()<1)return;
    	String string=ShowCity_lists.get(firstVisibleItem).getPinyi();
    	
		String[] strings=letterListView.b;
		String b="";
		for(int i=0;i<strings.length;i++){
		   b+=strings[i];
		}
		int number=b.indexOf(getAlpha(string));
		
		//Toast.makeText(SelectCityActivity.this,ShowCity_lists.size()+"" , 2000).show();
        if (!(number==letterListView.choose)) {
        	//Toast.makeText(SelectCityActivity.this,ShowCity_lists.size()+"" , 2000).show();
			letterListView.choose=number;
    ...
    private ArrayList<City> getCityList() {
        ...
        try {
            JSONArray chineseCities = new JSONArray(getResources().getString(R.string.citys));
    ...
    public class ListAdapter extends BaseAdapter {
        ...
        @Override
        public int getViewTypeCount() {// 这里�?要返回需要集中布�?类型，�?�大小为类型的种数的下标
            return VIEW_TYPE;
        }
...
//				if (sh.getText().length()==0) {//搜所状�??
//					holder.name.setText(list.get(position).getName());
//					holder.alpha.setVisibility(View.GONE);
//				}else if(position>0){
            //显示拼音和热门城市，�?次检查本次拼音和上一个字的拼音，如果�?样则不显示，如果不一样则显示
            ...
            String previewStr = (position - 1) >= 0 ? getAlpha(ShowCity_lists.get(position - 1).getPinyi()) : " ";//上一个拼�?
...
//				sb.append(location.getAddrStr());
    ...
    Handler handler2 = new Handler() {
        public void handleMessage(android.os.Message msg) {
            switch (msg.what) {
                case SHOWDIALOG:
                    progress = showProgress(SelectCityActivity.this, "正在加载数据，请稍等");
                    break;
                case DISMISSDIALOG:
                    if (progress != null) {
                        progress.dismiss();
                    ...
                    sh.addTextChangedListener(new TextWatcher() {
                        ...
                        public void afterTextChanged(Editable s) {
                            //搜索符合用户输入的城市名
                            if (s.length() > 0) {
                                ArrayList<City> change_city = new ArrayList<City>();
